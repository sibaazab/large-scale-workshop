...
func (obj *TestServiceClient) Store(ctx context.Context, key string, value string) error {
	...
	req := &service.StoreKeyValue{Value: value, Key: key}
...
func (obj *TestServiceClient) isAlive() (wrapperspb.BoolValue, error) {
	c, closeFunc, err := obj.Connect()
	if err != nil {
		return wrapperspb.BoolValue{}, fmt.Errorf("could not connect: %v", err)
	}
	defer closeFunc()

	r, err := c.IsAlive(context.Background(), &emptypb.Empty{})
	if err != nil {
		return wrapperspb.BoolValue{}, fmt.Errorf("could not call isAlive: %v", err)
	}
	return *r, nil
}
